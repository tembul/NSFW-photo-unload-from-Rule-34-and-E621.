    ...
    while True:
        ...
        try:
            ...
            for post in posts:
                ...
                try:
                    file_url = None
                    if post.get('file') and post['file'].get('url'):
...
if __name__ == '__main__':
    try:
        ...
        folder = input(f"{Colors.CYAN}Папка для сохранения (Enter для 'Крым море 2009'):{Colors.RESET} ").strip()
        if not folder:
            folder = 'Sisin_the_Great_2009'
